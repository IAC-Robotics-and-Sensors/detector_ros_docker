...
from gr1_driver.gr1_detector import GR1  # type: ignore[import]
...
class GR1Node(Node):
    def __init__(self):
        ...
        self.spectrum_pub = self.create_publisher(
            UInt32MultiArray, "/gr1/spectrum", 10
        )
        self.cps_pub = self.create_publisher(UInt32, "/gr1/cps", 10)

        self.logger = SpectrumCSVLogger(
            "gr1", log_dir=log_dir, enabled=log_enabled, logger=self
        )

        self.worker, q_list = GR1(verbose=verbose)
        ...
        self.get_logger().info(
            "GR1 ROS2 node started, publishing /gr1/spectrum and /gr1/cps; "
    ...
    def loop_once(self):
        ...
        if spectrum is None:
            self.get_logger().warn("GR1: no spectrum received within timeout")
            time.sleep(1.0)
            return
    ...
    def shutdown(self):
        self.get_logger().info("GR1 ROS2 node shutting down, sending quit to worker")
        try:
