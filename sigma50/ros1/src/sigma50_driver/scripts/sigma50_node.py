...
# The script lives in: <package root>/scripts/
THIS_DIR = os.path.dirname(os.path.abspath(__file__))
SRC_DIR = os.path.join(THIS_DIR, "..", "src")
if SRC_DIR not in sys.path:
    sys.path.insert(0, SRC_DIR)

from sigma50_driver.sigma_detector import Sigma50
...
def main():
    rospy.init_node("sigma50_node")

    spectrum_pub = rospy.Publisher("/sigma50/spectrum", UInt32MultiArray, queue_size=10)
    cps_pub = rospy.Publisher("/sigma50/cps", UInt32, queue_size=10)

    verbose = rospy.get_param("~verbose", 0)
    log_dir = rospy.get_param("~log_dir", "/data")
    log_enabled = rospy.get_param("~log_enabled", True)

    logger = SpectrumCSVLogger("sigma50", log_dir=log_dir, enabled=log_enabled)

    worker, q_list = Sigma50(verbose=verbose)
    cmd_q, data_q = q_list
    ...
    rospy.loginfo(
        "Sigma50 ROS node started, publishing /sigma50/spectrum and /sigma50/cps "
        "(integers); logging to %s",
        log_dir,
    )
    ...
    try:
        while not rospy.is_shutdown():
            ...
            if spectrum is None:
                rospy.logwarn_throttle(5.0, "Sigma50: no spectrum received within timeout")
                rate.sleep()
                continue
            ...
            logger.add_sample(delta_spectrum)
        ...
        rospy.loginfo("Sigma50 ROS node shutting down, sending quit to worker")
        try:
            cmd_q.put(["quit", None])
        except Exception:
            pass
