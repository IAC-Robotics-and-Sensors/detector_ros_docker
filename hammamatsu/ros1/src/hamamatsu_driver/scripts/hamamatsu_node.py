...
# -------------------------------------------------------------------
# Make sure Python can see the hamamatsu_driver package under src/
# -------------------------------------------------------------------
...
from hamamatsu_driver.hamamatsu_detector import Hamamatsu  # noqa: E402
...
def main():
    rospy.init_node("hamamatsu_node")

    spectrum_pub = rospy.Publisher(
        "/hamamatsu/spectrum", UInt32MultiArray, queue_size=10
    )
    cps_pub = rospy.Publisher("/hamamatsu/cps", UInt32, queue_size=10)

    verbose = rospy.get_param("~verbose", 0)
    log_dir = rospy.get_param("~log_dir", "/data")
    log_enabled = rospy.get_param("~log_enabled", True)

    logger = SpectrumCSVLogger("hamamatsu", log_dir=log_dir, enabled=log_enabled)

    # Hamamatsu() in hamamatsu_detector should return (worker_process, [cmd_q, data_q])
    worker, q_list = Hamamatsu(verbose=verbose)
    cmd_q, data_q = q_list
    ...
    rospy.loginfo(
        "Hamamatsu ROS node started, publishing /hamamatsu/spectrum and /hamamatsu/cps; "
        "logging to %s",
        log_dir,
    )
    ...
    try:
        while not rospy.is_shutdown():
            ...
            if spectrum is None:
                rospy.logwarn_throttle(
                    5.0, "Hamamatsu: no spectrum received within timeout"
                )
                rate.sleep()
                continue
        ...
        rospy.loginfo("Hamamatsu ROS node shutting down, sending quit to worker")
        try:
            cmd_q.put(["quit", None])
        except Exception:
            pass
